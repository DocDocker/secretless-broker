...
func createListenerOnPort(port string) (net.Listener, error) {
	...
	return listener, listener.(*net.TCPListener).SetDeadline(time.Now().Add(5 * time.Second))
}
...
func newMockTarget(port string) (*mockTarget, error) {
	listener, err := createListenerOnPort(port)
	if err != nil {
		return nil, err
	}
